    ...
    if cmdd:
        ...
        for command in cmd:
            ...
            if command.startswith('$DATA'):
                ...
                print(str(cmd.index(command))+'      <INDEX')
                print(cmd[cmd.index(command)]+'      <VALUE')
        ...
        if str(None) in str(cmd):
            ...
            return Data
        elif cmd[0] in Functions:
            temp = ''
            for x in range(1, len(cmd)):
                temp+=str(cmd[x]).strip()
            code = Functions[cmd[0]].replace('\n', '').split('then')
            for command in code:
                print(interactive(command.strip().split(' '), 0, True))
        else:
            print('''%s
